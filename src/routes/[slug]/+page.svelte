<script>
  ...
  import { next, prev, flat, flag, countryName, countryCodes, countryLabel } from '$lib/outline.js';
...
>
  ...
  <div class="body" bind:this={bodyEl}>
    <div class="meta vt-partlabel">
      ...
      {#if section.country}
        <span class="origin" title={countryLabel(section.country)}>
          <span class="flags" aria-hidden="true">
            {#each countryCodes(section.country) as c}
              <a class="flag-link" href="{base}/countries#{c}">{flag(c)}</a>
            {/each}
          </span>
          <span class="country-name">{countryLabel(section.country)}</span>
        </span>
      {/if}
      {#if section.class}
        <span class="class-tag class-{section.class.replace(/[^a-z]/gi, '').toLowerCase()}">{section.class}</span>
      {/if}
    </div>
    ...
    {#if section.cultural}
      <aside class="cultural">
        <div class="cultural-label">
          {#if section.country}
            <span class="cultural-flags" aria-hidden="true">
              {#each countryCodes(section.country) as c}<span>{flag(c)}</span>{/each}
            </span>
          {/if}
          <span>Cultural context · {countryLabel(section.country)}</span>
        </div>
        <p class="cultural-body">{@html md(section.cultural, mdOpts)}</p>
      </aside>
    {/if}
...
<style>
  ...
  .flags { display: inline-flex; gap: 0.25rem; }
  .flag-link { font-size: 1.4rem; line-height: 1; letter-spacing: 0; text-decoration: none; transition: transform 160ms ease; }
  .flag-link:hover { transform: scale(1.15); }
  .country-name { font-size: 0.66rem; }
  .cultural-flags { display: inline-flex; gap: 0.25rem; font-size: 1.1rem; line-height: 1; letter-spacing: 0; }
  ...
  .class-undecidable { color: #8a1c1c; }
  .class-npcomplete { color: #a64a00; }
  .class-nphard { color: #a66800; }
  .class-p { color: #1c5e2c; }
  .class-meta { color: var(--muted); }
  ...
  .cultural-body {
    font-family: var(--serif);
    font-weight: 300;
    font-size: clamp(0.92rem, 1.0vw, 1.0rem);
    line-height: 1.55;
    color: var(--ink);
  }
  ...
  @media (max-width: 720px) {
    ...
    .number, .title, .gesture, .body-text, .source, .cultural, .eli5 { grid-column: 1; max-width: none; }
    .title, .gesture, .body-text, .cite, .cultural-body, .eli5-body { overflow-wrap: break-word; word-break: break-word; hyphens: auto; }
